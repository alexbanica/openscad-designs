...
- Side USB-A center X positions sit on the left and right board-edge margin lines by default.
...
- Side USB-A connector depth runs along X and width runs along Y; each side connector center sits on the corresponding +/-X board-edge margin line.
...
- The top cover uses rooted short-end clip tabs and matching tray catch receivers; it does not use cover screws and it does not depend on the Pi Zero stack mounting screws.
...
- Side USB-A previews default to 13.2 mm x 5.7 mm footprints, use X as board length and Y as board width, and place center X on `-hat_board_half_length_mm` and `hat_board_half_length_mm`.
...
- Top cover is removable, clip-on, includes an alignment lip, rooted positive-retention clips on both short ends, adjustable ventilation/access holes, and does not depend on the Pi Zero stack mounting screws.
- Top-cover clips and tray-side receivers are visibly continuous with their parent solids by code review, do not read as floating or token-overlap geometry, and preserve material around the default camera cutout clearance.
