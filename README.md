...
- IR emitter pod board support, pilot-hole, and retainer parameters,
...
- `ir_pod`: the open-top external IR emitter pod body with its slide-slot attachment, solid board support bosses, cable entry, panel receiver sockets, and pass-through LED aperture, placed on its pod floor at Z=0. Standalone `ir_pod` mode shows the pod body only; the detachable top service panel and removable retainer are exported from `printable_layout`.
...
- With the Grove cable already connected, slide the IR emitter board through the cable-side pod service window, rest it on the pod bosses, align the LED to the pod aperture, route the cable through the pod entry without a sharp bend, fit the printed retainer through the service panel opening, then press the detachable top service panel into the pod and slide the pod onto the top-cover rails until the retention features engage.
- The removable printed retainer is an intentional secondary printable part for the enabled screw-retained PCB path, not an unexplained loose pod part. The detachable top service panel is a separate pod closure part, provides access to the pilot holes/screw locations, and does not retain the PCB by itself.
...
- The pod cable entry, IR LED aperture, pod board bosses, printed retainer, slide slots, and emitter reference preview all derive from the same adjustable pod/emitter pose.
